...
def main():
    ...
    if "Char usages for each stage" in run_commands:
        ...
        for room in ["1-1", "1-2", "2-1", "2-2", "3-1", "3-2", "4-1", "4-2", "5-1", "5-2", "6-1", "6-2", "7-1", "7-2", "8-1", "8-2", "9-1", "9-2", "10-1", "10-2"]:
            ...
            char_chambers[room] = char_chambers[room][RECENT_PHASE].copy()
        appearances = {}
        rounds = {}
        for room in char_chambers:
            appearances[room] = dict(sorted(char_chambers[room].items(), key=lambda t: t[1]["app"], reverse=True))
            rounds[room] = dict(sorted(char_chambers[room].items(), key=lambda t: t[1]["round"], reverse=False))
            for char in char_chambers[room]:
                appearances[room][char] = {
                    "app": char_chambers[room][char]["app"],
                    "rarity": char_chambers[room][char]["rarity"],
                    "diff": char_chambers[room][char]["diff"]
                }
                rounds[room][char] = {
                    "round": char_chambers[room][char]["round"],
                    "rarity": char_chambers[room][char]["rarity"],
                    "diff": char_chambers[room][char]["diff_rounds"]
                }
